...
def get_dataset_class(name):
    if name == "humanml":
        ...
        from closd.diffusion_planner.data_loaders.h1_prefix_dataset import get_h1_prefix_dataset
        return get_h1_prefix_dataset
    else:
...
def get_collate_fn(name, hml_mode='train', pred_len=0, batch_size=1):
    ...
    if name in ["humanml", "kit"]:
        ...
        # For H1 prefix dataset, always use prefix collate function
        return lambda x: t2m_prefix_collate(x, pred_len=pred_len if pred_len > 0 else 60)
    else:
        return all_collate
...
def get_dataset(name, num_frames, split='train', hml_mode='train', abs_path='.', fixed_len=0, hml_type=None, 
    ...
    if name in ["humanml", "kit"]:
        ...
        dataset = DATA(split=split, num_frames=num_frames, abs_path=abs_path, fixed_len=fixed_len, 
                       device=device, autoregressive=autoregressive, return_keys=return_keys, cache_path=cache_path)
    else:
...
def get_dataset_loader(name, batch_size, num_frames, split='train', hml_mode='train', hml_type=None, abs_path='closd/diffusion_planner', fixed_len=0, pred_len=0, 
    ...
    cache_path = None  # Simplified - no cache for now

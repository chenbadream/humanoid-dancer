...
from closd.diffusion_planner.data_loaders.humanml.utils.get_opt import get_opt
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        self.prompt_to_key = {} # FIXME: this dict becomes redundant once we return keys in __getitem__
        for key, data in self.data_dict.items():
            for t in data['text']:
                self.prompt_to_key[t['caption']] = self.prompt_to_key.get(t['caption'], []) + [key]
    ...
    def remove_motion(self, key):
        ...
        for t in self.data_dict[key]['text']:
            if len(self.prompt_to_key[t['caption']]) == 1:
                ...
                self.prompt_to_key[t['caption']] = self.prompt_to_key[t['caption']][:key_idx] + self.prompt_to_key[t['caption']][key_idx+1:]
    ...
    def remove_by_text(self, text):  # FIXME: this function becomes redundant once we return keys in __getitem__
        n_removed = 0
        for t in text:  
    ...
    def __getitem__(self, item):
        ...
        # print(tokens)
        ret = word_embeddings, pos_one_hots, caption, sent_len, motion, m_length, '_'.join(tokens)
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", hml_type=None, **kwargs):
        self.mode = mode
